...
-- Functions
--
CREATE DEFINER=`root`@`localhost` FUNCTION `TotalFacturadoPorProducto` (`producto_id` INT) RETURNS DECIMAL(10,2)  BEGIN
    DECLARE total DECIMAL(10, 2);
    
    SELECT 
        SUM(fp.Cantidad * p.PrecioUnit_Prod)
    INTO 
        total
    FROM 
        factura_producto fp
    JOIN 
        producto p ON p.ID_Prod = fp.ProductoID
    WHERE 
        p.ID_Prod = producto_id;
...
--
DELIMITER $$
CREATE TRIGGER `after_client_delete` AFTER DELETE ON `cliente` FOR EACH ROW BEGIN
    INSERT INTO log_cliente_deletion (RFC_Cte, Nombre_Cte, Fecha_Eliminacion, Deleted_By)
    VALUES (OLD.RFC_Cte, OLD.Nombre_Cte, NOW(), USER());
END
$$
DELIMITER ;
...
(41, '2024-12-03', '2024-12-06', 'DHL', 1.00, 'Pendiente', 'Centro'),
(42, '2024-12-06', '2024-12-09', 'DHL', 69.00, 'Pendiente', 'Robert'),
(43, '2024-12-06', '2024-12-09', 'DHL', 69.00, 'Pendiente', 'Robert'),
(44, '2024-12-06', '2024-12-09', 'DHL', 69.00, 'Pendiente', 'Robert');
...
(0, '2024-12-03', 201480.00, 32236.80, 233716.80, 'venta', 41, 1, 'GU000098'),
...
(41, '2024-12-03', 17618.00, 2818.88, 20436.88, 'venta', 42, 1, 'SJ000095'),
(42, '2024-12-03', 17618.00, 2818.88, 20436.88, 'venta', 43, 1, 'SJ000095'),
(43, '2024-12-03', 332125.00, 53140.00, 385265.00, 'venta', 44, 1, 'SJ000095');
...
--
-- Triggers `factura_producto`
--
DELIMITER $$
CREATE TRIGGER `after_factura_producto_insert` AFTER INSERT ON `factura_producto` FOR EACH ROW BEGIN
    UPDATE producto
    SET Cantidad_Prod = Cantidad_Prod - NEW.Cantidad
    WHERE ID_Prod = NEW.ProductoID;
END
$$
DELIMITER ;
...
--
-- Table structure for table `log_cliente_deletion`
--

CREATE TABLE `log_cliente_deletion` (
  `log_id` int(11) NOT NULL,
  `RFC_Cte` varchar(50) DEFAULT NULL,
  `Nombre_Cte` varchar(100) DEFAULT NULL,
...
(1, 'adidas tenis Gazelle Indoor', 'Earth Strata/Cloud White/Magic Beige - 25% extra', -19, 2920.00, 730.00, 'adidas_gazelle.jpg'),
(2, 'Onitsuka Tiger tenis Mexico 66', 'Cream Mineral - Elección responsable', 26, 8809.00, 0.00, 'onitsuka_mexico66.jpg'),
(3, 'Jordan tenis Air Jordan 1', 'Low OG Velvet Brown de JORDAN x Travis Scott', 20, 25330.00, 0.00, 'jordan_airjordan1.jpg'),
(4, 'AMIRI tenis Pacific', 'Nueva temporada', 0, 13285.00, 0.00, 'amiri_pacific.jpg'),
...
--
-- Indexes for table `log_cliente_deletion`
--
ALTER TABLE `log_cliente_deletion`
...
--
-- AUTO_INCREMENT for table `log_cliente_deletion`
--
ALTER TABLE `log_cliente_deletion`
...
ALTER TABLE `factura`
  ...
  ADD CONSTRAINT `factura_ibfk_3` FOREIGN KEY (`ClienteRFC`) REFERENCES `cliente` (`RFC_Cte`) ON DELETE CASCADE;
